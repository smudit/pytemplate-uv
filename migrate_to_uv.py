...
def create_backup(project_path: Path) -> Path:
    ...
    backup_path = (
        project_path.parent / f"{project_path.name}_backup_{int(__import__('time').time())}"
    )
...
def update_pyproject_toml(
    project_path: Path, config: dict, features: dict, dry_run: bool = False
) -> list[str]:
    ...
    if config.get("build-system", {}).get("build-backend") != "hatchling.build":
        ...
        if not dry_run:
            config["build-system"] = {"requires": ["hatchling"], "build-backend": "hatchling.build"}
    ...
    if "project" in config and "dependencies" in config["project"]:
        ...
        for dep in deps:
            ...
            elif dep.startswith("pydantic") and ">=2." not in dep:
